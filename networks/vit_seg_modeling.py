...
class Embeddings(nn.Module):
    ...
    def __init__(self, config, img_size, in_channels=3):
        ...
        #这里的Conv是用于生成Transformer的patch块
        self.patch_embeddings = Conv2d(in_channels=in_channels,
                                       out_channels=config.hidden_size,
                                       kernel_size=patch_size,
                                       stride=patch_size)  # 初始化patch嵌入层
        self.position_embeddings = nn.Parameter(torch.zeros(1, n_patches, config.hidden_size))  # 初始化位置嵌入E_pos
    ...
    def forward(self, x):
        if self.hybrid:  # 如果使用混合模型
            x, features = self.hybrid_model(x)  # 获得混合模型的输出
        else:
